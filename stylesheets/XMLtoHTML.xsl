...
<!-- 																			-->
<!-- 2016.04.01 - Fixed error with file and item levels in container list       -->
<!-- 2016.04.21 - Added expand/collapse all function for descriptions           -->
<!-- 2016.04.22 - physdesc and scopecontent now concatenated in descriptions    -->
<!-- 2016.04.28 - Added handling for items inside a file and expand-all js      -->
<!-- **************************************************************************	-->
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:php="http://php.net/xsl" xmlns="http://www.w3.org/TR/REC-html40">
	...
	<html><head>
		<xsl:processing-instruction name="php">include "/afs/umbc.edu/public/web/sites/library/prod/htdocs/boiler_sc_xml_header.txt";?</xsl:processing-instruction>
				...
				<script type="text/javascript" src="http://library.umbc.edu/speccoll/findingaids/expandcollapseall.js"/>
            </head>
            <body>
					...
					<div id="secondbox">
						...
						<xsl:if test="archdesc/dsc">
							...
							<div class="section6">
								...
								<xsl:if test="archdesc/dsc//*[@level='file']/scopecontent | archdesc/dsc//*[@level='item']/scopecontent">
									<span class="expand"><a href="javascript:unhideall('hidden');">Expand all descriptions</a> | <a href="javascript:hideall('unhidden');">Collapse all</a></span>
								</xsl:if>
								<xsl:choose> 
									...
									<xsl:when test="./*[@level]">
										<ol class="bodylist"><li>
											<table id="folder">
												...
												<xsl:apply-templates select="archdesc/dsc/*[@level='file'] | archdesc/dsc/*[@level='item']"/>
											</table>
										</li></ol>
									</xsl:when>
								</xsl:choose>
...
<xsl:processing-instruction name="php">include "/afs/umbc.edu/public/web/sites/library/prod/htdocs/speccoll/sc_xml_footer.txt";?</xsl:processing-instruction>
   ...
   <xsl:template match="archdesc/dsc//*[@level='series'] | archdesc/dsc//*[@level='subseries']">
                ...
                <xsl:when test="./*[@level]">
                    <ol class="bodylist"><li>
                        <table id="folder">
                            ...
                            <xsl:apply-templates select="./*[@level='file'] | ./*[@level='item']"/>
                        </table>
                    </li></ol>
                </xsl:when>
            </xsl:choose>
	...
	<xsl:template match="archdesc/dsc//*[@level='file'] | archdesc/dsc//*[@level='item']">
        <tr>
			<td>
				<xsl:choose>
                <xsl:when test="..[@level='file']/*[@level]">
					<xsl:attribute name="class">subitem</xsl:attribute>
			...
			<td>
				<xsl:if test="./did/dao/descriptivenote">
				<a>
					<xsl:attribute name="href" select="./did/dao/@href"/>
					<xsl:value-of select="./did/dao/descriptivenote/p"/>
				</a><br/>
				</xsl:if>
				<xsl:if test="./scopecontent | ./did/physdesc">
		...
		<xsl:if test="./scopecontent | ./did/physdesc">
			<tr>
				<td colspan="5">
				<xsl:variable name="num2" select="generate-id()"/>
				<div id="{$num2}" class="hidden">
					<xsl:choose>
						<xsl:when test="./did/physdesc">
							<xsl:value-of select="concat(./did/physdesc, '. ', ./scopecontent)"/>
						</xsl:when>
						<xsl:otherwise>
